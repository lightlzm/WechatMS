						...
						<div class="weui-desktop-layout__main__inner">
							<div class="main_bd">
								...
								<div class="weui-desktop-panel weui-desktop-panel_overview">
									<h3 class="weui-desktop-panel__title">添加关键词回复</h3>
                                    ...
                                    <input type="text" class="weui-desktop-form__input" id="kwd" value="<? echo $row['kwd'];?>">
                                </div>
									<!-- 群发信息的类型 -->
									<div class="title_tab">
			...
			$(".submit-btn").click(function(){ 
				...
				if(nowstyle=='joinroom'){
					...
					$.ajax({
						type: "GET",
						url: "<? echo API_URL;?>", 
						data: "action=add_auto_reply&name=join_room&bid=<? echo $bid;?>&roomid="+roomid+"&kwd="+encodeURIComponent(kwd), 
						dataType: 'jsonp',
						timeout: 10000,
						jsonp: "callback",
			    ...
			    if(nowstyle=='urlLink'){ 
					...
					$.ajax({
						type: "GET",
						url: "<? echo API_URL;?>", 
						data: "action=add_auto_reply&name=send_urlLink&bid=<? echo $bid;?>&title="+encodeURIComponent(urlLink_title)+"&description="+encodeURIComponent(urlLink_desc)+"&url="+encodeURIComponent(urlLink_url)+"&thumbnailUrl="+encodeURIComponent(urlLink_image_url)+"&kwd="+encodeURIComponent(kwd), 
						dataType: 'jsonp',
						timeout: 10000,
						jsonp: "callback",
						success: function(result){
				...
				if(nowstyle=='image'){
					...
					$.ajax({
						type: "GET",
						url: "<? echo API_URL;?>", 
						data: "action=add_auto_reply&name=send_image&bid=<? echo $bid;?>&url="+encodeURIComponent(url)+"&kwd="+encodeURIComponent(kwd), 
						dataType: 'jsonp',
						timeout: 10000,
						jsonp: "callback",
						success: function(result){
			    ...
			    if(nowstyle=='text'){ 
					...
					$.ajax({
						type: "GET",
						url: "<? echo API_URL;?>", 
						data: "action=add_auto_reply&name=send_text&bid=<? echo $bid;?>&text="+encodeURIComponent(text)+"&kwd="+encodeURIComponent(kwd), 
						dataType: 'jsonp',
						timeout: 10000,
						jsonp: "callback",
						success: function(result){
