...
<body>
						...
						<div class="weui-desktop-layout__main__inner">
							<div class="main_bd">
								...
								<div class="weui-desktop-panel weui-desktop-panel_overview" style="padding-top:30px;">
                                    ...
                                    <div class="tmplmsg__search-wrap weui-desktop-data-overview-list" style="width: 600px;margin-bottom: 10px;display: flex;  padding:10px; border:1px solid #ff6a00; background-color: #FFE8D0;border-radius: 3px; line-height:1.5rem; font-size:12px;">
                                       温馨提示：<br>
                                       为了降低风险并保证系统稳定运行，我们对小助手的推送频率和推送 量做了以下调整：<br>
                                       ①定时群发和即时群发 15 分钟内只能创建一条任务。
                                    </div>
                                    ...
                                    <div class="tmplmsg__search-wrap weui-desktop-data-overview-list" style="width: 600px;margin-bottom: 10px;display: flex">
                                        <label style="min-width:120px;line-height: 36px;color: #666;">提醒标题：</label>
                                        <input id="title" type="text" class="weui-desktop-form__input info-title" placeholder="最多30个字符">
                                    </div>
                                    <div class="tmplmsg__search-wrap weui-desktop-data-overview-list" style="width: 600px;margin-bottom: 10px;display: flex">
											<label style="min-width:120px;line-height: 36px;color: #666;">提醒时间：</label>
											<div style="color: #999;display: inline-block;cursor: pointer;line-height: 36px;margin-right: 20px;width:78px" data-type="定时群发" class="set-time">
												<div class="weui-desktop-icon-checkbox on" style="margin-top: -2px;margin-right: 2px"></div>
												定时群发
											</div>
											<div style="color: #999;display: inline-block;cursor: pointer;line-height: 36px;" data-type="重复提醒" class="set-time">
												<div class="weui-desktop-icon-checkbox" style="margin-top: -2px;margin-right: 2px"></div>
												重复提醒
											</div>
					                    </div>
					                     <div class="select-date">
					                    	<div class="input-wrap date-wrap" style="margin-left: 120px">
					                    		<span class="calendar-icon"></span>
					                    		<input type="text" class="weui-desktop-form__input " placeholder="2020-01-01 12:00" id="date" value="<? echo date("Y-m-d H:i",strtotime("+10 minutes"))?>">
					                    	</div>
					                    	<div class="week-wrap" style="margin-left: 120px">
					                    		<div class="week-list">
					                    			<span  weekday="1">周一</span>
					                    			<span  weekday="2">周二</span>
					                    			<span  weekday="3">周三</span>
					                    			<span  weekday="4">周四</span>
					                    			<span  weekday="5">周五</span>
					                    			<span  weekday="6">周六</span>
					                    			<span weekday="0">周日</span>
					                    		</div>
					                    		<div class="input-wrap week-time">
						                    		<span class="clock-icon"></span>
						                    		<input type="text" class="weui-desktop-form__input " placeholder="12:00" id="time" value="<? echo date("H:i",strtotime("+10 minutes"))?>">
						                    	</div>
					                    	</div>
    ...
    <script src="<? echo _URL_;?>assets/lib/laydate/laydate.js"></script> 
	<script>
	    //执行一个laydate实例
		laydate.render({
		  	elem: '#date' //指定元素
		...
		$(function(){
			...
			$(".set-time").click(function(){
				...
				$(".set-time").find(".on").removeClass("on")
				$(this).find("div").addClass("on")
			...
			$(".week-list span").click(function(){
				...
				weekArr =[]
				var weekSelect = $(".week-list .on") 
				for(var x = 0;x<weekSelect.length;x++){ 
					weekArr.push($(weekSelect[x]).attr("weekday"))
				}
                
				weekday=weekArr.join(",")
				console.log(weekday)
			...
			$(".submit-btn").click(function(){
			    ...
			    if(nowstyle=='urlLink'){ 
					...
					$.ajax({
						type: "GET",
						url: "<? echo API_URL;?>", 
						data: "action=create_job&name=send_chatroom_urlLink&bid=<? echo $bid;?>&title="+encodeURIComponent(urlLink_title)+"&roomid="+roomid+"&description="+encodeURIComponent(urlLink_desc)+"&url="+encodeURIComponent(urlLink_url)+"&thumbnailUrl="+encodeURIComponent(urlLink_image_url)+"&at_all="+at_all+"&title="+encodeURIComponent(title)+"&send_time="+send_time+"&weekday="+weekday+"&send_style="+send_style, 
						dataType: 'jsonp',
						timeout: 10000,
						jsonp: "callback",
						success: function(result){
							 ...
							 setTimeout(function(){
							     hidePopSuccess()
								 location="index.php?action=room_send_message_timeing_list&bid=<? echo $bid;?>";
							 }, 1000 * 2 );
						},
						error: function(msg){ 
						     hideSquareLoading();
				...
				if(nowstyle=='image'){
					...
					$.ajax({
						type: "GET",
						url: "<? echo API_URL;?>", 
						data: "action=create_job&name=send_chatroom_image&bid=<? echo $bid;?>&url="+encodeURIComponent(url)+"&roomid="+roomid+"&at_all="+at_all+"&title="+encodeURIComponent(title)+"&send_time="+send_time+"&weekday="+weekday+"&send_style="+send_style, 
						dataType: 'jsonp',
						timeout: 10000,
						jsonp: "callback",
						success: function(result){
			    ...
			    if(nowstyle=='text'){ 
					...
					$.ajax({
						type: "GET",
						url: "<? echo API_URL;?>", 
						data: "action=create_job&name=send_chatroom_text&bid=<? echo $bid;?>&text="+encodeURIComponent(text)+"&roomid="+roomid+"&at_all="+at_all+"&title="+encodeURIComponent(title)+"&send_time="+send_time+"&weekday="+weekday+"&send_style="+send_style, 
						dataType: 'jsonp',
						timeout: 10000,
						jsonp: "callback",
						success: function(result){
